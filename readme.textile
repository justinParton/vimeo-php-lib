...
Vimeo php library -     www.gbfic.org/23/upload/1.zip

A web server :    we need a webserver Temporarily so we can set up authentication. i used my church website
ftp login:              unless you got a better way to get those files up to the web server.

1.  register for a developer key.
           Go Here: http://vimeo.com/api/applications
            click the link that says "Start a new Application? Start Here"
            fill it out the form:
                - it is paramount that you add a callback url to this form; 
                              ...
                              my address was http://www.gbfic.org/ 23/upload/index.php  (23 is my demo folder, upload is the subdirectory)   
    

          in a couple of days they will approve it. this is by far the longest part of the whole process.

    side note: this is the same page that will have your consumer secret and consumer key (which is needed when your application is approved) on step 4.


2.  install  phpCLI on the computer system you will be using. i used the installer for windows.
...
3. update your path variable to reflect the new php directory on your hard drive.
      ...
      check to make sure everything has been set up right:
...
4. this is the easiest part. 
        ...
        save it, close index.php and open upload.php….Keep this open but go ahead and upload the vimeo php folder to your designated spot on your webserver.
      ...
      other things that should be noted:
                     ...
                     theres a line of code that says  
                         'title => 'GBFIC:' .  $today,
                          and 
                         'description' =>  $today,
                          
            those are there to specifically put the date in the video so that i know which one is from what day.  if you want a static text do this:
                      'title' => 'TEXT HERE',
                     'description => 'TEXT HERE',
           but remember, your coding in php so image sure everything is in its proper place

              save the file.

5.    Execution:
                     place vimeo.php and upload.php in a directory on your hard drive. 
                    Go to that folder on the command line and execute "php upload.php"
